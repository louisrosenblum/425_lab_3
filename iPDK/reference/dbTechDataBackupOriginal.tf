...
leMPPControls (
leMPPDefinition( psubGR
	( ptypeGuardRing)
  ( 1)	0 ) ;leMPPDefinition


leMPPRingObject (
	ptypeGuardRing defaultPath (diffEncPath pplusEncPath ) nil  (defaultContacts ) nil  vss 
	userParams( name netName )
) ;leMppRingObject
leMPPDefinition( nwellGR
	( ntypeGuardRing)
  ( 1)	0 ) ;leMPPDefinition


leMPPRingObject (
	ntypeGuardRing defaultPath (diffEncPath nplusEncPath nwellEncPath ) nil  (defaultContacts ) nil  vdd 
	userParams( name netName )
) ;leMppRingObject
leMPPMasterPath (
	defaultPath M1                drawing           0.05 extend t t
	userParams( name width layer purpose conn chop )
)
leMPPEnclosedPath (
	diffEncPath DIFF              drawing           0.016 extend t nil
	userParams( name enclosure layer purpose conn chop )
)
leMPPEnclosedPath (
	pplusEncPath PIMP              drawing           0.06 extend t nil
	userParams( name enclosure layer purpose conn chop )
)
leMPPEnclosedPath (
	nplusEncPath NIMP              drawing           0.06 extend t nil
	userParams( name enclosure layer purpose conn chop )
)
leMPPEnclosedPath (
	nwellEncPath NWELL             drawing           0.1 extend t nil
	userParams( name enclosure layer purpose conn chop )
)
leMPPSubRect (
	defaultContacts CO               drawing           0.042 0.042 t t 0.004 0.035 0.05 fit 1
...
(POLYCON          PO           M1           ( CO          0.042 0.042 0 ) 
	 (1 1 ( 0.05 0.05 )  )
...
( validRoutingVias	( VIA12 VIA12BAR VIA12LG VIA12C VIA12BARC VIA12LGC VIA23 VIA23BAR VIA23LG VIA23C VIA23BARC VIA23LGC VIA34 VIA34BAR VIA34LG VIA34C VIA34BARC VIA34LGC POLYCON DIFFCON VIA45 VIA45BAR VIA45LG VIA56 VIA56BAR VIA56LG VIA67 VIA67BAR VIA67LG VIA78 VIA78BAR VIA78LG VIA89 ) )
 );LEFDefaultRouteSpec
) ;constraintGroups
